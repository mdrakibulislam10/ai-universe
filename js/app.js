  ...
  document.getElementById("tool-container").innerHTML = `
       <div class="card w-100 rounded-4">
           <div class="card-body bg-danger bg-opacity-10 border border-danger rounded-4">
               <h5 class="card-title">${description ? description : "No Data Found"}</h5>
              <div class="d-flex flex-column flex-md-row justify-content-between gap-2 w-100 my-5">
                ...
                <span class="">${!pricing || (pricing[0]?.price) === "0" || (pricing[0]?.price).includes("No cost") ? "Free of Cost/" : pricing[0]?.price}</span> <br />
                 <span class="">${!pricing || pricing[0]?.plan === "Free" ? "Basic" : pricing[0]?.plan}</span>
                 </div>
                 <div class="text-center p-2 bg-white rounded-3 fw-semibold" style="color: #F28927;">
                 <span class="">${!pricing || (pricing[1]?.price) === "0" || (pricing[1]?.price).includes("No cost") ? "Free of Cost/" : pricing[1]?.price}</span> <br />
                 <span class="">${!pricing ? "Pro" : pricing[1]?.plan}</span>
                 </div>
                 <div class="text-center p-2 bg-white rounded-3 text-danger fw-semibold">
                 <span class="">${pricing ? (pricing[2]?.price).slice(0, 10) : "Free of Cost /"}</span> <br />
                 <span class="">${pricing ? pricing[2]?.plan : "Enterprise"}</span>
                 </div>
               </div>
